...
def get_college_categories(request, college_id):
    ...
    This endpoint returns all course categories available for the selected college
    """
    try:
        college = College.objects.get(college_id=college_id)
        
        # Get categories from courses_offered JSONField
...
def get_category_degree_types(request, college_id, category):
    ...
    This endpoint returns all degree types (UG, PG, Diploma, etc.) for the selected college and category
    """
    try:
        college = College.objects.get(college_id=college_id)
        
        # Validate category is offered by college
        ...
        # Get unique degree types for this college and category
...
def get_degree_courses(request, college_id, category, degree_type):
    ...
    This endpoint returns all available courses for the selected combination
    """
    try:
        college = College.objects.get(college_id=college_id)
        ...
        # Get courses matching all criteria
...
def get_all_colleges_with_categories(request):
    ...
    Useful for populating the college dropdown with category counts
    """
    try:
        ...
        for college in colleges:
            ...
            category_names = [
                ...
                for cat in categories_offered[:3]  # Show first 3 categories
            ]
...
def get_college_hierarchy(request, college_id):
    """
    Get complete hierarchy for a college (all categories, degree types, and courses)
    This is useful for populating the entire form at once
    """
    try:
        college = College.objects.get(college_id=college_id)
        ...
        for category_code in categories_offered:
            ...
            # Get degree types for this category
            degree_types = Course.objects.filter(
                college_id=college_id,
                category=category_code,
                is_active=True
            ...
            for degree_code in degree_types:
                ...
                # Get courses for this degree type
                courses = Course.objects.filter(
                    college_id=college_id,
                    category=category_code,
                    degree_type=degree_code,
...
# ==================== ENHANCED APPLICATION SUBMISSION WITH SELECTED COURSE ====================

@api_view(['POST'])
@permission_classes([IsAuthenticated])
def submit_application_v2(request):
    """
    Enhanced version of submit_application that uses the selected_course foreign key
    This ensures proper data integrity and supports the 4-step selection process
    """
    try:
        user = request.user
        print(f"Processing application for user: {user.id} - {user.username}")

        # Get the selected course ID (new way)
