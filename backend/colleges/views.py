...
# ============================================================
# 3. HIERARCHICAL SELECTION VIEWS (College → Category → Degree → Course)
# ============================================================
...
# ============================================================
# 6. APPLICATION FORM VIEWS (PUBLIC - NO AUTH REQUIRED)
# ============================================================

@api_view(['GET'])
def get_application_form_data(request):
    """Get application form data for pre-filling (NO AUTH REQUIRED)"""
    try:
...
def submit_application(request):
    ...
    try:
        ...
        # Handle college_id to college conversion
        college_id = data.get('college_id') or data.get('college')
        if college_id and not data.get('college'):
            try:
                college = College.objects.get(college_id=college_id)
                data['college'] = college.college_id
            except College.DoesNotExist:
                pass

        # Handle selected_course if present
        selected_course_id = data.get('selected_course_id') or data.get('selected_course')
        if selected_course_id and not data.get('selected_course'):
            try:
                selected_course = Course.objects.get(course_id=selected_course_id, is_active=True)
                data['selected_course'] = selected_course.course_id
                ...
                if not data.get('college'):
                    data['college'] = selected_course.college.college_id
            except Course.DoesNotExist:
                pass

        # Use the serializer for proper validation and creation
        serializer = EnquiryFormCreateSerializer(data=data)
...
def submit_application_simple(request):
    """Simple version of submit_application - NO AUTHENTICATION REQUIRED"""
    try:
        user = request.user if request.user.is_authenticated else None
        ...
        if user:
            ...
            application_id = f'APP-{clean_identifier}-{datetime.now().strftime("%Y%m%d%H%M%S")}'
        ...
        selected_course_id = request.data.get('selected_course_id') or request.data.get('selected_course')
        if selected_course_id:
            try:
                selected_course = Course.objects.get(course_id=selected_course_id, is_active=True)
                if not college and selected_course:
        ...
        application = EnquiryForm.objects.create(
            ...
            has_diploma=request.data.get('has_diploma', False),
            diploma_marks_percentage=request.data.get('diploma_marks_percentage') or None,
            has_ug=request.data.get('has_ug', False),
...
def submit_application_v2(request):
    ...
    return submit_application_simple(request)


# ============================================================
# 7. APPLICATION RETRIEVAL VIEWS (AUTHENTICATED & GUEST)
# ============================================================

@api_view(['GET'])
def get_my_applications(request):
    """Get all enquiry forms for the authenticated user"""
    try:
        user = request.user if request.user.is_authenticated else None
...
def get_application_detail(request, application_id):
    """Get a specific enquiry form by ID (authenticated users only)"""
    try:
        user = request.user if request.user.is_authenticated else None
...
@api_view(['POST'])
def get_applications_by_contact(request):
    """Get applications by email or mobile number (for guests to check their applications)"""
    try:
        email = request.data.get('email_id')
        mobile = request.data.get('mobile_number')

        if not email and not mobile:
            return Response({'error': 'Email or mobile number required'}, status=400)
...
@api_view(['GET'])
def get_application_detail_by_contact(request, application_id):
    """Get a specific enquiry form by ID (works for guests with verification)"""
    try:
        email = request.query_params.get('email')
        mobile = request.query_params.get('mobile')

        application = EnquiryForm.objects.get(application_id=application_id)

        # Verify access - either authenticated user owns it, or email/mobile matches
        user_owns = request.user.is_authenticated and application.user == request.user
        contact_matches = (email and application.email_id == email) or (mobile and application.mobile_number == mobile)

        if not (user_owns or contact_matches):
...
# ============================================================
# 8. USER PROFILE VIEWS (AUTHENTICATED)
# ============================================================
