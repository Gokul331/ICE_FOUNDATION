...
import { getCollegeDetail, getCollegeCourses, getCollegeFees } from '../services/api';
          ...
          <div className="college-hero-content">
            ...
            <div className="college-title-section">
              ...
              <div className="college-meta">
                ...
                {college.affiliation && <span className="meta-item">{college.affiliation}</span>}
              </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '16px' }}>
                        ...
                        {college.website && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <a href={college.website} target="_blank" rel="noopener noreferrer" style={{ color: '#000', textDecoration: 'none' }}>
                              {college.website}
                            </a>
                          </div>
                        )}
                ...
                <div className="info-card">
                          ...
                          <tr key={course.course_id || index}>
                            <td className="course-name">
                              ...
                              {course.specialization && (
                                <div style={{ fontSize: '12px', color: '#666' }}>{course.specialization}</div>
                              )}
                            </td>
                            ...
                            <td>
                              ...
                              {fees.find(f => f.course_id === course.course_id)?.fee_per_year && (
                                <div style={{ fontSize: '12px', color: '#666' }}>
                                  ₹{fees.find(f => f.course_id === course.course_id).fee_per_year.toLocaleString()}/year
                                </div>
                              )}
                            </td>
                            <td>
                              <button className="apply-now-btn" onClick={() => alert(`Applied to ${course.course_name}`)}>
                                Apply
                              </button>
                            </td>
                          </tr>
                  ...
                  {fees.length > 0 && (
                    <div style={{ marginTop: '32px' }}>
                      <h4>Fee Breakdown</h4>
                      {fees.map((fee, index) => {
                        const course = courses.find(c => c.course_id === fee.course_id);
                        return (
                          <div key={index} style={{ marginBottom: '24px', padding: '16px', background: '#fafafa', borderRadius: '12px' }}>
                            <h5 style={{ marginBottom: '12px' }}>{course?.course_name || 'Course'} - Fee Structure</h5>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
                              {fee.tuition_fee && (
                                <div><strong>Tuition Fee:</strong> {formatCurrency(fee.tuition_fee)}</div>
                              )}
                              {fee.admission_fee && (
                                <div><strong>Admission Fee:</strong> {formatCurrency(fee.admission_fee)}</div>
                              )}
                              {fee.caution_deposit && (
                                <div><strong>Caution Deposit:</strong> {formatCurrency(fee.caution_deposit)}</div>
                              )}
                              {fee.library_fee && (
                                <div><strong>Library Fee:</strong> {formatCurrency(fee.library_fee)}</div>
                              )}
                              {fee.lab_fee && (
                                <div><strong>Lab Fee:</strong> {formatCurrency(fee.lab_fee)}</div>
                              )}
                              {fee.hostel_fee && (
                                <div><strong>Hostel Fee:</strong> {formatCurrency(fee.hostel_fee)}</div>
                              )}
                              {fee.mess_fee && (
                                <div><strong>Mess Fee:</strong> {formatCurrency(fee.mess_fee)}</div>
                              )}
                              {fee.transport_fee && (
                                <div><strong>Transport Fee:</strong> {formatCurrency(fee.transport_fee)}</div>
                              )}
                              {fee.total_fees && (
                                <div><strong>Total Fees:</strong> {formatCurrency(fee.total_fees)}</div>
                              )}
                            </div>
                          </div>
                        );
          ...
          {activeTab === 'admissions' && (
            <div className="info-card">
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <p>{college.eligibility_criteria || 'Contact college for detailed eligibility criteria.'}</p>
              </div>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'grid', gap: '12px' }}>
                  {college.application_deadline && (
                    <div><strong>Application Deadline:</strong> {new Date(college.application_deadline).toLocaleDateString()}</div>
                  )}
                  {college.entrance_exam_date && (
                    <div><strong>Entrance Exam Date:</strong> {new Date(college.entrance_exam_date).toLocaleDateString()}</div>
                  )}
                  {college.counseling_date && (
                    <div><strong>Counseling Date:</strong> {new Date(college.counseling_date).toLocaleDateString()}</div>
                  )}
                </div>
              </div>
          ...
          {activeTab === 'facilities' && (
            <div className="info-card">
              ...
              <div>
                ...
                <p>{college.infrastructure_details || 'Modern infrastructure with state-of-the-art facilities.'}</p>
              </div>
            </div>
          )}
            ...
            <div>
              <div className="info-card">
                ...
                <div>
                  ...
                  <p>{college.placement_process || 'The college has a dedicated placement cell that organizes campus recruitment drives.'}</p>
                </div>
              </div>

              {/* Recent Placements */}
              {college.recent_placements && college.recent_placements.length > 0 && (
